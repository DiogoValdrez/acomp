...
## Programa principal--x^y--convencional(out)--x(a0),y(a1)
	lw a0, x
	lw a1, y #PC xc
	jal Pow #ra x14
...
## Pow--x^y--convencional(in)-convencional(out)--a0(x),a1(y)
Pow:
	addi sp, sp, -12
	sw ra, 8(sp)
	sw s1, 4(sp)
...
## Mult--x^y--convencional(in)-convencional(out)--a0(x),a1(y)
